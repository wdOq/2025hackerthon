...
def save_output_files(out_records, output_file, final_dir: Optional[str] = None, final_cid: Optional[str] = None):
    ...
    # Optionally write a final file named by date + CID into final_dir
    if final_dir and final_cid:
        try:
            from datetime import datetime
            date_str = datetime.now().strftime('%Y%m%d')
            final_name = f"{date_str}_{final_cid}.json"
            final_path = Path(final_dir) / final_name
            final_path.parent.mkdir(parents=True, exist_ok=True)
            with open(final_path, 'w', encoding='utf-8') as f:
                json.dump(out_records, f, ensure_ascii=False, indent=2)
            print(f"Final JSON written to: {final_path}")
        except Exception as e:
            print(f"[WARN] Failed to write final JSON: {e}")
...
def parse_args():
    ...
    parser.add_argument("--cid", help="Chemical CID to use in final file name (e.g., 1131)")
    parser.add_argument("--final_dir", default="final_output", help="Directory to write final date+CID named JSON")
...
def main():
    ...
    if args.process_all:
        ...
        if not filtered_records:
            ...
            # Still create empty output file and final file (if cid/final_dir provided)
            try:
                save_output_files([], args.output_file, final_dir=args.final_dir, final_cid=(args.cid or None))
            except Exception:
    ...
    # Keep original order of filtered_records
    ...
    # Save output files (and optional final copy)
    save_output_files(out_records, args.output_file, final_dir=args.final_dir, final_cid=(args.cid or None))
